...
def main():
    ...
    inference_df = pd.read_csv(
        "attentive_fp_random/regression_inference_results/prediction.csv"
    )
    ...
    for t, strucid_df in df.groupby(["strucid", "scoring_function"]):
        ...
        parent_pred = strucid_df[strucid_df["is_parent"] == True]["prediction"].values[
            0
        ]
    ...
    df.loc[
        (df["ligand_id"].str.contains("inter") == False)
        & (df["ligand_id"].str.contains("intra") == False),
        "category",
    ] = "contact"
        ...
        for strucid in strucids:
            if strucid not in type_df["strucid"].to_list():
                type_df = type_df.append(
                    {"strucid": strucid, "pIC50 change": np.nan}, ignore_index=True
                )
