...
import pandas as pd
from ccdc import descriptors, io, protein
...
for protein_file in Path(protein_path).glob("*dry.mol2"):
    ...
    ligand_file = str(
        list(Path(ligand_path).glob(f"*{ccdc_protein.identifier}*.sdf"))[0]
    )
    ccdc_ligand = io.MoleculeReader(ligand_file)[0]
    ccdc_protein.add_ligand(ccdc_ligand)
    amide_n = [
        a
        for a in ccdc_protein.atoms
        if "GLN726" in a.residue_label and a.label == "NE2"
    ][0]
    ...
    close_atoms = [
        ...
        if a.protein_atom_type == "Ligand" and a.atomic_symbol != "H"
    ]
    distances = []
    for close_atom in close_atoms:
        distances.append(
            descriptors.MolecularDescriptors.atom_distance(amide_n, close_atom)
        )
    if distances:
