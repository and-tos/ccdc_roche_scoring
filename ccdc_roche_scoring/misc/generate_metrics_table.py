...
def add_attentive_fp(attentive_fp_df, predictions_df, dgl_path, extension=""):
    ...
    predictions_df[hybrid_column_name] = (
        predictions_df["RF-PLP"] + predictions_df[afp_column_name]
    ) / 2
...
class TableMaker(object):
    def __init__(self):
        ...
        vanilla_df = vanilla_df.sort_values(
            "Gold.PLP.Fitness", ascending=False
        ).drop_duplicates("SRN")
    ...
    def get_predictions_df(
        self,
        dgl_path=Path("dgl_models_random"),
        ...
        train_srns = dataset_df[dataset_df["split"].isin(["val", "train"])][
            "SRN"
        ].unique()
        ...
        train_set_mean = dataset_df[dataset_df["split"].isin(["val", "train"])][
            "pde10_h_pic50"
        ].mean()
        ...
        vanilla_test_df = vanilla_test_df.rename(
            columns={"Gold.PLP.Fitness": "vanilla_Gold.PLP.Fitness"}
        )
        ...
        vanilla_train_df = vanilla_train_df.join(
            dataset_df[dataset_df["SRN"].isin(train_srns)][
                ["SRN", "pde10_h_pic50"]
            ].set_index("SRN"),
            on="SRN",
        )
        linear_fit = LinearRegression().fit(
            vanilla_train_df["Gold.PLP.Fitness"].to_numpy().reshape(-1, 1),
        ...
        template_df = template_df.rename(
            columns={
                "Gold.PLP.Fitness": "template_Gold.PLP.Fitness",
                "rescore": "RF-PLP",
            }
        )
        ...
        extended_df_path = (
            dgl_path / "attentive_fp_extended_training_set/predictions.csv"
        )
        if extended_df_path.is_file():
            attentive_fp_df = pd.read_csv(extended_df_path)
            predictions_df = add_attentive_fp(
                attentive_fp_df, predictions_df, dgl_path, extension="_ed"
            )
        return predictions_df
    ...
    def make_table(
        ...
        predictions_df = self.get_predictions_df(dgl_path, rf_score_path)
        ...
        for model in predictions_df.columns:
            ...
            if model == "train_set_mean":
                ...
                table_dict["spearman"].append(
                    round(return_spearmanr(label, predictions), 2)
                )
                table_dict["spearman_ci"].append(
                    return_ci(label, predictions, return_spearmanr)
                )
                table_dict["r2"].append(round(return_r2(label, predictions), 2))
                table_dict["r2_ci"].append(return_ci(label, predictions, return_r2))

            if "Gold" in model:
                table_dict["rmse"].append(np.nan)
                table_dict["rmse_ci"].append(np.nan)
            else:
                table_dict["rmse"].append(round(return_rmse(label, predictions), 2))
                table_dict["rmse_ci"].append(return_ci(label, predictions, return_rmse))

            table_dict["model"].append(model)
        table_df = pd.DataFrame(table_dict)
        table_df = table_df[
            ["model", "spearman", "spearman_ci", "rmse", "rmse_ci", "r2", "r2_ci"]
        ]
...
def main():
    ...
    # Table 2
    table_maker.make_table(
        dgl_path=Path("dgl_models_aminohetaryl_c1_amide"),
        rf_score_path=Path(
            "docking_template_date/first_iteration_aminohetaryl_c1_amide"
        ),
        output=Path("table3_aminohetaryl_c1_amide_with_avg_scaled.csv"),
    )

    table_maker.make_table(
        dgl_path=Path("dgl_models_c1_hetaryl_alkyl_c2_hetaryl"),
        rf_score_path=Path(
            "docking_template_date/first_iteration_c1_hetaryl_alkyl_c2_hetaryl"
        ),
        output=Path("table4_c1_hetaryl_alkyl_c2_hetaryl_with_avg_scaled.csv"),
    )

    table_maker.make_table(
        dgl_path=Path("dgl_models_aryl_c1_amide_c2_hetaryl"),
        rf_score_path=Path(
            "docking_template_date/first_iteration_aryl_c1_amide_c2_hetaryl"
        ),
