...
import pandas as pd
from ccdc import entry, io, protein
...
def _get_alignment_templates(target="pde-10", project_dir=Path(".")):
    structure_quality = pd.read_csv(f"{target}_structure_quality.csv")
    high_quality_strucs = structure_quality[
        (structure_quality["RESOLUTION"] <= 2.5)
        & (structure_quality["DENSITYFLAGS"] <= 5)
        & (structure_quality["LIGCLASHES"] == 0)
        & (structure_quality["LIGSYMCONTACT"] == 0)
    ]["STRUCID"].to_list()
    assayed_structures = list((project_dir / target / "tmp_2d_sdf").glob("*.sdf"))
    ligand3d_sdf = list((project_dir / target / "tmp_aligned_3d_sdf").glob("*.sdf"))
    # assayed_3d_structures = []
...
def _get_pdb_ligname_df(target="pde-10", project_dir=Path(".")):
    pdb_files = list((project_dir / target / "tmp_aligned_for_MOE").glob("*.pdb"))
    sdf_files = list((project_dir / target / "tmp_2d_sdf").glob("*.sdf"))
    ligand3d_sdf = list((project_dir / target / "tmp_aligned_3d_sdf").glob("*.sdf"))
...
def _sanitize_pdb_files(target="pde-10", scoring_dir=Path(".")):
    pdb_files = list((scoring_dir / target / "tmp_aligned_for_MOE").glob("*.pdb"))
    sanitized_path = scoring_dir / target / "tmp_aligned_for_MOE_sanitized"
    ...
    for pdb_file in pdb_files:
        ...
        sanitized_file = sanitized_path / Path(
            pdb_file.name.replace(".pdb", "_sanitized.pdb")
        )
...
def _get_binding_site_residues(target="pde-10", scoring_dir=Path(".")):
    sanitized_path = scoring_dir / target / "tmp_aligned_for_MOE_sanitized"
            ...
            with io.EntryReader(pdb_file) as rdr:
                for e in rdr:
                    ...
                    ligands = [
                        ...
                        if c.atoms[-1].residue_label[:3] == ligname
                    ]
                    if not ligands:
                        continue
                    if len(ligands) != 1:
                        ...
                        ligand = ligands[0]
                    if (
                        not 9 < len(ligand.atoms) < 500
                    ):  # kick out fragments and covalently bound ligands
                        continue
                    for l in p.ligands:
                        p.remove_ligand(l.identifier)
                    for a in p.atoms:
    ...
    binding_site = (
        binding_site["residue_identifier"]
        .str.split(":", expand=True)
        .drop_duplicates(subset=1)
        .rename(columns={0: "chain_label", 1: "residue_identifier"})
    )
...
def _export_ligands(target="pde-10"):
    ...
    for cnt, protein_file in enumerate(protein_files):
        ...
        ligand_name = pdb_ligand_df[pdb_ligand_df["strucid"] == strucid][
            "ligand"
        ].values[0]
        ...
        for r in ccdc_protein.residues:
            if (
                r.chain_identifier != "A"
                and r.atoms[0].protein_atom_type == "Amino_acid"
            ):
...
def main():
    ...
    scoring_dir = Path("rf_scoring")
    # print('Getting alignment templates...')
    _get_alignment_templates(args.target, project_dir)

    # print('Assigning ligand names to PDB files...')
    _get_pdb_ligname_df(args.target, project_dir)

    # print('Sanitizing PDB files...')
    _sanitize_pdb_files(args.target, scoring_dir)

    # print('Getting bindingsite residues...')
    _get_binding_site_residues(args.target, scoring_dir)
