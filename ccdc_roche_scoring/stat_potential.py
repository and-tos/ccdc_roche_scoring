...
def parse_args():
    ...
    parser.add_argument("--target", help="Target name.", default="pde-10")

    parser.add_argument(
        "--task", help="Name of the pIC50 attribute.", default="pde10_h_pic50"
    )
    ...
    parser.add_argument(
        "-df",
        "--dataframe",
        help="DataFrame containing columns 'Split' and 'SRN'. Only SRNs that are labeled as train or val will be used for fitting",
        default=False,
    )
...
def hbond_compensation(ligand_contacts):
    ...
    ligand_contacts.loc[
        (ligand_contacts["ligand_atom_symbol"]).isin(["S", "Cl", "F", "I"])
        & (
            ligand_contacts["rf_total"]
            < 1
            & ligand_contacts["protein_atom_type"].isin(
                ["O_ali_mix", "O_pi_mix", "Water"]
            )
        ),
        "interaction_type",
    ] = "desolvation"
    ...
    ligand_contacts.loc[
        (ligand_contacts["ligand_atom_symbol"]).isin(["S", "Cl", "F", "I"])
        | (ligand_contacts["los_atom_symbol"]).isin(["S", "Cl", "F", "I"]),
        "is_hbond",
    ] = False
    ...
    ligand_contacts.loc[
        (ligand_contacts["is_hbond"] == False)
        & (ligand_contacts["interaction_type"] == "hbond_classic")
        & (ligand_contacts["is_hbond_mismatch"] == False),
        "interaction_type",
    ] = "desolvation"
    ...
    ligand_contacts.loc[
        (ligand_contacts["is_hbond"] == True)
        & (ligand_contacts["los_atom_symbol"] == "C"),
        "interaction_type",
    ] = "hbond_weak"
    ...
    ligand_contacts.loc[
        ligand_contacts["interaction_type"] == "hbond_classic", "rm"
    ] = (ligand_contacts["rm"] - 0.3)
    ...
    ligand_contacts.loc[
        (ligand_contacts["interaction_type"] == "hbond_weak")
        & (ligand_contacts["is_hbond"] == True)
        & (
            ligand_contacts["protein_atom_type"].isin(
                ["O_ali_mix", "O_pi_mix", "Water"]
...
def turn_off_secondary_clash_contacts(ligand_contacts):
    ...
    for ligand_file, df in ligand_contacts.groupby("ligand_file"):
        ...
        clashing_atom_contact_indices = df[
            (df["ligand_atom_index"].isin(clashing_atom_indices))
            & (df.index.isin(clashing_df_indices) == False)
        ].index

        secondary_clash_contact_indices.extend(clashing_atom_contact_indices)

    ligand_contacts["clash_count"] = (
        ligand_contacts[ligand_contacts["is_clash"] == True]
        .groupby(["ligand_file", "ligand_atom_index"])[["ligand_atom_index"]]
        .transform("count")
    )
...
def prepare_contact_df(ligand_contacts, count_df):
    ...
    ligand_contacts["interaction_type"] = ligand_contacts["interaction_type"].fillna(
        "hydrophobic"
    )
    ...
    for symbol in radii:
        ligand_contacts.loc[
            ligand_contacts["los_atom_symbol"] == symbol, "los_atom_radius"
        ] = radii[symbol]
        ligand_contacts.loc[
            ligand_contacts["ligand_atom_symbol"] == symbol, "ligand_atom_radius"
        ] = radii[symbol]
    ligand_contacts["rm"] = (
        ligand_contacts["ligand_atom_radius"] + ligand_contacts["los_atom_radius"]
    )
    ...
    for index, row in count_df[
        count_df["ligand_file"].isin(ligand_contacts["ligand_file"])
    ].iterrows():
        ...
        ligand_contacts.loc[
            ligand_contacts["ligand_file"] == ligand_file, "pc_frozen_bonds"
        ] = pc_frozen_bonds
...
def process_df(strain=["ANI_Strain"], task="pde10_h_pic50"):
    ...
    if not Path("best_soln_contacts.csv").is_file():
        contact_df = pd.concat(
            map(pd.read_csv, Path(".").glob("docking_job_*/docked_contact_df.csv")),
            ignore_index=True,
        )
        contact_df["interaction_type"] = contact_df["interaction_type"].str.replace(
            "halogen_bond", "multipolar"
        )
        contact_df = contact_df[contact_df["is_intramolecular"] == False]

        contact_df = contact_df.drop(
            columns=[c for c in contact_df.columns if "Gold.Protein" in c]
        )
        contact_df = contact_df.drop(
            columns=[
                c
                for c in contact_df.columns
                if c
                in [
                    "Gold.Version",
    ...
    ligand_contacts_file = "best_soln_contacts_processed.csv"
    if Path(ligand_contacts_file).is_file():
        ligand_contacts = pd.read_csv(ligand_contacts_file)

    else:
        # count_df_file = Path("../docked_rf_count_best_docked.csv")
        count_df_file = Path("gold_plp_fitness.csv")
        ...
        ligand_contacts = ligand_contacts[ligand_contacts["RMSD_to_mcs"] <= 1.00]
        ##
        count_df["ligand_file"] = (
            count_df["ligand_file"].str.split("/", expand=True)[12]
            + "/"
            + count_df["ligand_file"].str.split("/", expand=True)[13]
        )
        ligand_contacts["ligand_file"] = (
            ligand_contacts["ligand_file"].str.split("/", expand=True)[12]
            + "/"
            + ligand_contacts["ligand_file"].str.split("/", expand=True)[13]
        )
        ##
        ligand_contacts = ligand_contacts[
            ligand_contacts["ligand_file"].isin(count_df["ligand_file"])
        ]
        ...
        if "ANI_Strain" in strain:
            ...
            ligand_contacts = ligand_contacts[
                ligand_contacts["ANI_Strain"].isna() == False
            ]
...
class PlpScoring(object):
    def __init__(self, task, strain=[], series="", gold=True):
        ...
        params = ["w"]
        interaction_type_params = [
            a + "_" + b for a, b in itertools.product(self.interaction_types, params)
        ]
        ...
        fav_params = [
            a + "_" + b
            for a, b in itertools.product(fav_interaction_type_groups, params)
        ]
        ...
        params = (
            ...
            + [
                a + "_" + b
                for a, b in itertools.product(unfav_interaction_type_groups, params)
            ]
        )

        self.interaction_type_groups = (
            fav_interaction_type_groups + unfav_interaction_type_groups
        )
    ...
    def plp_scoring_function(self, x, ligand_contact_df, return_contact_df=False):
        ...
        ligand_contact_df["plp_energy"] = 0.0
        ...
        for interaction_ in int_dict:
            if interaction_ == "unfavorable":
                ...
                interaction_indices = ligand_contact_df[
                    ligand_contact_df["interaction_type"].isin(int_dict[interaction_])
                ].index
                rm = ligand_contact_df.loc[interaction_indices]["rm"]
                A_unfav = pd.concat([A_unfav, (rm - a)])
                B_unfav = pd.concat([B_unfav, (rm + c)])

                # F_unfav is positive for RF_total < 1
                _c = -(np.log(ligand_contact_df.loc[interaction_indices]["rf_total"]))
                F_unfav = pd.concat([F_unfav, _c])
                ...
                E = pd.concat(
                    [
                        ...
                        -(
                            np.log(
                                ligand_contact_df.loc[interaction_indices]["rf_total"]
                            )
                        )
                        + e,
                    ]
        ...
        favorable_contacts = ligand_contact_df[
            ligand_contact_df["interaction_type"].isin(int_dict["unfavorable"]) == False
        ].copy()
        distances = favorable_contacts["distance"].sort_index()
        clash_counts = favorable_contacts["clash_count"].sort_index()
        favorable_contacts.loc[distances.lt(A), "plp_energy"] = (
            clash_factor * (clash_counts + 1) * ((A - distances) / A) + clash_factor
        )
        favorable_contacts.loc[
            ((distances[A.index] >= A) & (distances[B.index] < B)).index, "plp_energy"
        ] = (E * (distances - A) / (B - A))
        favorable_contacts.loc[
            ((distances[B.index] >= B) & distances.lt(C)).index, "plp_energy"
        ] = E
        favorable_contacts.loc[
            ((distances[C.index] >= C) & (distances.lt(D))).index, "plp_energy"
        ] = (E * (D - distances) / (D - C))

        unfavorable_contacts = ligand_contact_df[
            ligand_contact_df["interaction_type"].isin(int_dict["unfavorable"]) == True
        ].copy()
        clash_counts = unfavorable_contacts["clash_count"].sort_index()
        distances = unfavorable_contacts["distance"].sort_index()
        unfavorable_contacts.loc[distances.lt(A_unfav), "plp_energy"] = (
            clash_factor * (clash_counts + 1) * ((A_unfav - distances) / A_unfav)
            + clash_factor
        )
        unfavorable_contacts.loc[
            ((distances[A_unfav.index] >= A_unfav) & distances.lt(B_unfav)).index,
            "plp_energy",
        ] = (
            -F_unfav * (distances - A_unfav) / (B_unfav - A_unfav) + F_unfav
        )

        ligand_contact_df = pd.concat([favorable_contacts, unfavorable_contacts])

        for interaction_type, ligand_interaction_df in ligand_contact_df.groupby(
            "interaction_type"
        ):
            if interaction_type in ["hbond_classic", "ionic"]:
                ...
                ligand_interaction_df.loc[
                    buriedness < params_dict["hbond_rho1"], "plp_energy"
                ] = 0

                ligand_interaction_df.loc[
                    (params_dict["hbond_rho1"] <= buriedness)
                    & (buriedness < params_dict["hbond_rho2"]),
                    "plp_energy",
                ] = (
                    ligand_interaction_df["plp_energy"]
                    * (buriedness - params_dict["hbond_rho1"])
                    / (params_dict["hbond_rho2"] - params_dict["hbond_rho1"])
                )
            ...
            if interaction_type in [
                ...
                ligand_interaction_df.loc[
                    buriedness < params_dict["lipophilic_rho1"], "plp_energy"
                ] = 0
                ligand_interaction_df.loc[
                    (params_dict["lipophilic_rho1"] <= buriedness)
                    & (buriedness < params_dict["lipophilic_rho2"]),
                    "plp_energy",
                ] = (
                    ligand_interaction_df["plp_energy"]
                    * (buriedness - params_dict["lipophilic_rho1"])
                    / (params_dict["lipophilic_rho2"] - params_dict["lipophilic_rho1"])
                )

            ligand_contact_df.loc[
                ligand_interaction_df.index, "plp_energy"
            ] = ligand_interaction_df["plp_energy"]
        ...
        ligand_contact_df.loc[
            ...
            & (bfactors < params_dict["bfactor_rho2"]),
            "plp_energy",
        ] = (
            ligand_contact_df["plp_energy"]
            * (params_dict["bfactor_rho2"] - bfactors)
            / (params_dict["bfactor_rho2"] - params_dict["bfactor_rho1"])
        )

        ligand_contact_df.loc[bfactors >= params_dict["bfactor_rho2"], "plp_energy"] = 0

        clash_contact_indices = list(
            unfavorable_contacts[unfavorable_contacts["distance"].lt(A_unfav)].index
        ) + list(favorable_contacts[favorable_contacts["distance"].lt(A)].index)
        ligand_contact_df["interaction_type"] = ligand_contact_df[
            "interaction_type"
        ].str.replace("halogen_bond", "multipolar")
        for interaction_type, interaction_df in ligand_contact_df[
            ligand_contact_df.index.isin(clash_contact_indices) == False
        ].groupby("interaction_type"):
            ligand_contact_df.loc[interaction_df.index, "plp_energy"] = (
                interaction_df["plp_energy"] * params_dict[interaction_type + "_w"]
            )
        ligand_contact_df.loc[
            ligand_contact_df.index.isin(clash_contact_indices), "plp_energy"
        ] = -ligand_contact_df["plp_energy"]
        column_labels = ["ligand_file", self.task, "pc_frozen_bonds"] + self.strain
        if self.gold:
            column_labels.append("Gold.PLP.Fitness")

        prediction_df = ligand_contact_df[column_labels].drop_duplicates("ligand_file")

        predicted_activities = ligand_contact_df.groupby(["ligand_file"])[
            "plp_energy"
        ].sum()
        ...
        for cnt, strain_label in enumerate(self.strain):
            ...
            if strain_label == "ANI_Strain":
                prediction_df["strain_penalty"] = params_dict[
                    f"strain_weight_{cnt}"
                ] * np.log(prediction_df[strain_label])
                prediction_df["strain_penalty"] = prediction_df[
                    "strain_penalty"
                ].replace([np.inf, -np.inf], 0)
            else:
                prediction_df["strain_penalty"] = (
                    prediction_df["strain_penalty"]
                    + params_dict[f"strain_weight_{cnt}"] * prediction_df[strain_label]
                )

        prediction_df["rotation_entropy_penalty"] = (
            params_dict["rotation_entropy_w"] * prediction_df["pc_frozen_bonds"]
        )
        prediction_df["score"] = (
            prediction_df["plp_energy"]
            + prediction_df["strain_penalty"]
            + prediction_df["rotation_entropy_penalty"]
        )
    ...
    def optimize_weights(self, scores_df):
        self.initial_guess = {
            "pi_w": -0.06329918369899717,
            "hydrophobic_w": -0.03679486613866424,
            "hbond_weak_w": -0.043416752578643,
            "multipolar_w": -0.019966358787951787,
            "repulsive_w": -0.04166345024299469,
            "desolvation_w": -0.009318217735359038,
            "hbond_classic_w": -0.2285049973821565,
            "ionic_w": -0.1765648797949396,
            "favorable_lip_a": 0.09999493880055929,
            "favorable_lip_b": 0.39996346524216725,
            "favorable_lip_c": 0.7999992895585842,
            "favorable_lip_d": 0.49999927117641335,
            "favorable_lip_e": -1.601062326025244,
            "favorable_hb_a": 0.09351018567138517,
            "favorable_hb_b": 0.26874110245376265,
            "favorable_hb_c": 0.4710463226463769,
            "favorable_hb_d": 0.2947095948102579,
            "favorable_hb_e": -0.40114233297001367,
            "unfavorable_a": 0.29990714415021325,
            "unfavorable_c": 0.5005102385721926,
            "hbond_rho1": 50.45643860461278,
            "hbond_rho2": 57.452535372621725,
            "lipophilic_rho1": 19.205766641846076,
            "lipophilic_rho2": 21.41581509284962,
            "bfactor_rho1": 0.1618446393425923,
            "bfactor_rho2": 0.1639701197217107,
            "strain_weight_0": -0.0007038247138676942,
            "strain_weight_1": -0.11715414788931568,
            "strain_weight_2": -5.154179015906084e-09,
            "rotation_entropy_w": -3.527789958418665e-06,
        ...
        scores_df = scores_df.drop(
            columns=[
                ...
                if "Gold." in c and c not in self.strain + ["Gold.PLP.Fitness"]
            ]
        )
        ...
        ret = minimize(
            ...
            method="L-BFGS-B",
            x0=list(self.initial_guess),
            args=(scores_df,),
            bounds=(self.bounds_dict.values()),
            tol=1e-03,
        )  # , tol=1e-05
        ...
        ret = least_squares(
            ...
            args=(scores_df,),
            bounds=(
                np.array(list(self.bounds_dict.values()))[:, 0],
                np.array(list(self.bounds_dict.values()))[:, 1],
            ),
            ftol=1e-03,
        )  # , ftol=1e-05
...
def rescore_prep(contact_scores_files):
    # RESCORE
    print("Rescoring poses...")
    scores_df = pd.concat(
        [
            pd.read_csv(f)
            for f in Path("..").glob("docking_job_*/rescored_rf_contact.csv")
        ],
        ignore_index=True,
    )
    scores_df = scores_df[scores_df["RMSD_to_mcs"] < 1.5]
    ligand_files_remove = []
    for index, df in scores_df.sort_values(by="rescore", ascending=False).groupby(
        "SRN"
    ):
    ...
    scores_df = scores_df.drop(
        columns=[c for c in scores_df.columns if "Gold.Protein" in c]
    )
    scores_df = scores_df.drop(
        columns=[
            c
            for c in scores_df.columns
            if c in ["Gold.Version", "Gold.Chemscore.Hbonds", "Gold.Score"]
        ]
    )
    scores_df.to_csv(contact_scores_files, index=False)


def write_results(contacts_df, series, task, prediction_df):
    contacts_df.to_csv(f"contacts_{series}.csv", index=False)

    final_prediction_df = prediction_df.copy()
    final_prediction_df["prediction_error"] = (
        final_prediction_df["score"] - final_prediction_df[task]
    )

    final_prediction_df = final_prediction_df.join(
        contacts_df[["SRN", "RMSD_to_mcs", "ligand_file"]]
        .drop_duplicates("ligand_file")
        .set_index("ligand_file"),
        on="ligand_file",
    )
    ...
    print(
        "spearman R RF rescore",
        spearmanr(final_prediction_df["score"], final_prediction_df[task]),
    )
    print(
        "spearman R Gold.PLP.Fitness",
...
def main():
    ...
    strain = [
        ...
        "Gold.PLP.Chemscore.Protein.Energy",
    ]
    ...
    if not args.rescore:
        ...
        rescore_prep(contact_scores_files)

    scores_df = pd.read_csv(contact_scores_files)

    # series_file = Path("../../series_assignment.csv")
    series_file = Path("gold_plp_fitness.csv")

    if series_file.is_file() and not args.dataframe:
        series_df = pd.read_csv(series_file)
        series_df = series_df[["SRN", "series"]]
        if "series" in scores_df.columns:
            scores_df = scores_df.drop(columns="series").join(
                series_df.set_index("SRN"), on="SRN"
            )
        else:
            scores_df = scores_df.join(series_df.set_index("SRN"), on="SRN")
    elif args.dataframe and Path(args.dataframe).is_file():
        series_df = pd.read_csv(args.dataframe)
        train_val_srns = series_df[series_df["split"].isin(["train", "val"])][
            "SRN"
        ].to_list()
    ...
    for series in scores_df["series"].unique():
        ...
        prediction_df, contacts_df = optimizer.plp_scoring_function(
            weights, series_df, return_contact_df=True
        )

        write_results(contacts_df, series, task, prediction_df)
