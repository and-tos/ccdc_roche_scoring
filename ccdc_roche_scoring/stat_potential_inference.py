...
from ccdc_roche_scoring import (
    descriptor_dfs,
    scoring_parameters,
    stat_potential,
    docking,
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="""
        Calculate RF-PLP score.
        """,
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,  # To display default values in help message.
    )

    parser.add_argument("-t", "--target", help="Target name.", default="pde-10")

    parser.add_argument("--task", help="Task name.", default="")

    parser.add_argument("-s", "--series", help="Series name.", default="")

    parser.add_argument("-l", "--ligand", help="Ligand file.", default="")

    parser.add_argument("-p", "--protein", help="Protein file.", default="")

    parser.add_argument(
        "-g", "--gold", action="store_true", help="Input is Gold solution file"
    )

    parser.add_argument("-df", "--dataframe", default=False, help="Input is CSV file")

    parser.add_argument("-d", "--docking_path", help="docking_path.", default=".")
...
class StatPotInferer:
    def __init__(
        ...
        gold=True,
        protein=False,
        mol_contact_df=pd.DataFrame(),
        rf_count_df=pd.DataFrame(),
        scoring_parameter_file=Path("."),
        task="pde10_h_pic50",
    ):
        self.target = target
        self.target_home = f"{target}_bindingsites"
        ...
        if scoring_parameter_file == Path("."):
            scoring_parameter_file = (
                Path("").resolve().parent / f"second_iteration/weights_{series}.json"
            )
            if not scoring_parameter_file.is_file():
                scoring_parameter_file = (
                    Path("").resolve().parent / f"first_iteration/weights_{series}.json"
                )
            if not scoring_parameter_file.is_file():
                scoring_parameter_path = Path(scoring_parameters.__path__._path[0])
                scoring_parameter_file = (
                    scoring_parameter_path / self.target / f"weights_{series}.json"
                )
    ...
    def _return_dfs(self):
        if self.gold:
            (
                rf_count_df,
                mol_contact_df,
            ) = descriptor_dfs.return_contact_df_from_docking_file(
                self.ligand_file,
    ...
    def _return_stat_pot_score(self):
        self.mol_contact_df = stat_potential.prepare_contact_df(
            self.mol_contact_df, self.rf_count_df
        )
        self.mol_contact_df = self.mol_contact_df.drop(
            columns=[
                c
                for c in self.mol_contact_df.columns
                if "Gold.PLP.part" in c or c in ["Gold.Version"]
            ]
        )
        ...
        prediction_df, self.mol_contact_df = optimizer.plp_scoring_function(
            self.weights, self.mol_contact_df, return_contact_df=True
        )
        ...
        if self.task in self.rf_count_df.columns:
            self.rf_count_df["error"] = (
                self.rf_count_df["rescore"] - self.rf_count_df[self.task]
            )
            self.mol_contact_df["error"] = (
                self.mol_contact_df["rescore"] - self.mol_contact_df[self.task]
            )
            ...
            for ligand in ligands:
                ...
                if "series" in ligand.attributes.keys():
                    ...
                    scoring_parameter_file = (
                        ...
                        / f"scoring_parameters/{args.target}/weights_{ligand_series}.json"
                    )

                else:
                    with open(
                        Path(__file__).parent
                        / Path(f"series_definitions/{args.target}.json")
                    ) as series_defs:
                    ...
                    for series in series_dict:
                        ...
                        hits = searcher.search(ligand_mol)
                        if hits:
                            ...
                            if not scoring_parameter_file.is_file():
                                scoring_parameter_file = (
                                    Path(__file__).parent
                                    / f"scoring_parameters/{args.target}/weights_train.json"
                                )
                            break
                if ligand_series:
                    print("scoring...")
                    scorer = StatPotInferer(
                        args.target,
                        ligand_series,
                        temp_ligand_file,
                        task=args.task,
                    ...
                    if scorer.score:
                        ...
                        ligand.attributes["RF Score"] = round(
                            rf_count_df["rescore"].values[0], 1
                        )
                        ligand.attributes["series"] = ligand_series
                        output_ligand_writer.write(ligand)
                else:
                    print("No scoring function for this series.")
    return rf_count_df_list, mol_contact_df_list
...
def main():
    ...
    if not args.dataframe:
        print("Score from ligand...")
        rf_count_df_list, mol_contact_df_list = score_pose(args)

    # Score from Contact DF
    elif args.dataframe:
        print("Score from DF...")
        rf_count_df_list, mol_contact_df_list = score_df(args)

    if rf_count_df_list:
        rf_count_df = pd.concat(rf_count_df_list, ignore_index=True)
        rf_count_df = rf_count_df.drop(
            columns=[c for c in rf_count_df.columns if "Gold.Protein" in str(c)]
        )
        rf_count_df = rf_count_df.drop(
            columns=[c for c in rf_count_df.columns if "Chemscore" in str(c)]
        )
        ...
        mol_contact_df = mol_contact_df.drop(
            columns=[c for c in mol_contact_df.columns if "Gold.Protein" in str(c)]
        )
        mol_contact_df = mol_contact_df.drop(
            columns=[c for c in mol_contact_df.columns if "Chemscore" in str(c)]
        )
    ...
    #     with io.EntryWriter("rescored_ligands.sdf") as w, io.EntryReader(
    #         args.ligand
    #     ) as rdr:
    #         for e in rdr:
