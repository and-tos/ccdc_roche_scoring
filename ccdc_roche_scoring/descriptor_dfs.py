...
"""
Generate DataFrame with RF descriptors of project data to available crystal structures.
"""
...
import pandas as pd
from ccdc import io
from rf_statistics import los_descriptors, rf_assignment
...
def parse_args():
    ...
    parser.add_argument(
        "--aligned_ligands", help="SDF file with aligned ligands", default=False
    )
...
def _group_by_template(aligned_molecules, project_home):
    ...
    with io.EntryReader(aligned_molecules) as rdr:
        ...
        for ligand in rdr.entries():
            ...
            pdb = pdb_ligand_df[pdb_ligand_df["ligand"] == ligand_query]["pdb"].values[
                0
            ]
            pdb = str(
                Path(project_home)
...
def return_contact_df_from_docking_file(
    docked_ligand_file,
    target_home="",
    strucid="",
    pdb_file=None,
    ...
    gold_conf = str(list(Path(docked_ligand_file).parents[0].glob(f"**/gold.conf"))[0])

    if pdb_file is None:
        pdb_file = list(
            (Path("../..") / Path("tmp_aligned_for_MOE_sanitized")).glob(
                f"{strucid}*.pdb"
            )
        )
        if pdb_file:
            ...
            pdb_file = list((Path(target_home)).glob(f"{strucid}*prot3d.pdb"))
            if pdb_file:
                pdb_file = pdb_file[0]
            else:
                pdb_file = None

    assigner = rf_assignment.RfAssigner(
        str(docked_ligand_file),
        gold=gold_conf,
        only_binding_site=False,
        intramolecular_protein_contacts=False,
        bfactor=pdb_file,
    ...
    mol_contact_df = mol_contact_df.drop(
        columns=[
            c
            for c in mol_contact_df.columns
            if "Gold.Protein" in c and c != "Gold.PLP.Chemscore.Protein.Energy"
        ]
    )
    if pdb_file:
        mol_contact_df["unique_protein_atom_label"] = (
            mol_contact_df["los_atom_residue_label"]
            + "_"
            + mol_contact_df["los_atom_label"]
        )
        bfactor_df = assigner.describer.bfactor_df
        bfactor_df["unique_protein_atom_label"] = (
            bfactor_df["residue_label"] + "_" + bfactor_df["normalised_atom_label"]
        )
        mol_contact_df = mol_contact_df.join(
            bfactor_df[["unique_protein_atom_label", "relative_bfactor"]].set_index(
                "unique_protein_atom_label"
            ),
            on="unique_protein_atom_label",
        )
    ...
    rf_score = np.log(
        mol_contact_df[
            (mol_contact_df["rf_total"] > 0)
            & (mol_contact_df["is_intramolecular"] == False)
        ]["rf_total"]
    ).sum()
...
def return_contact_df(docking_dir="", target_home=""):
    ...
    for docked_ligand_file in docked_structures:
        ...
        rf_count_df, mol_contact_df = return_contact_df_from_docking_file(
            docked_ligand_file, strucid=strucid, target_home=target_home
        )
...
def main():
    ...
    args = parse_args()
    if args.docking_dir:
        if args.target in ["pde10"]:
            target_home = f"{args.target}_bindingsites/"
        else:
            target_home = ""
        contact_df, rf_count_df = return_contact_df(args.docking_dir, target_home)

        rf_count_df.columns = [str(c) for c in rf_count_df.columns]
        rf_count_df = rf_count_df.drop(
            columns=[
                c
                for c in rf_count_df.columns
                if "Gold.Protein" in c or "Gold.Chemscore.Hbonds" in c
            ]
        )
        rf_count_df.to_csv(
            str(Path(args.docking_dir) / Path("docked_rf_count_df.csv")), index=False
        )
        ...
        contact_df.to_csv(
            str(Path(args.docking_dir) / Path("docked_contact_df.csv")), index=False
        )
