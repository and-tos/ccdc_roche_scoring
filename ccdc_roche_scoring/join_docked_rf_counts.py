...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="""
        Combine DataFrames from template docking in preparation for RF-PLP fitting.
        """,
...
def get_best_docking_solutions(df, rescoring=False):
    if rescoring:
        df = df[
            df["series"].isin(["aryl_piperazinone", "bcp", "hhpo", "hhpo_2", "spiro"])
        ]
    ...
    filtered_df = filtered_df[
        (filtered_df["RMSD_to_mcs"] <= 1.5) & (filtered_df["rel_mcs_size"] >= 0.5)
    ]
...
def concatenate_rf_counts(args):
    ...
    if "decoy" in str(dfs[0]):
        ...
        for df in dfs:
            if "TanimotoCombo_to_scaffold" in df.columns:
                if "TanimotoCombo_to_reference" in df.columns:
                    ...
                    df = df.rename(
                        columns={
                            "TanimotoCombo_to_scaffold": "TanimotoCombo_to_reference"
                        }
                    )
        ...
        # filtered_df = df.sort_values(by=['rel_mcs_size', 'Gold.PLP.Fitness'], ascending=[False, True]).drop_duplicates(
        #     subset='SRN')
        filtered_df = df.sort_values(
            by=["rel_mcs_size", scoring_function], ascending=[False, True]
        )
